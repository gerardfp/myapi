...
public class ActorController {
    ...
    public List<ActorDetails> getAll() {
        return mapStructMapper.actorsToActorsDetails(actorRepository.findAll());
    }

    @GetMapping("/{id}")
    public ActorDetails getById(@PathVariable UUID id) {
        return mapStructMapper.actorToActorDetails(actorRepository.findById(id).orElse(null));
    }

    @GetMapping("/findByName")
    public List<ActorDetails> findByName(@RequestParam String name) {
        return mapStructMapper.actorsToActorsDetails(actorRepository.findByName(name));
    }

    @GetMapping("/searchByName")
    public List<ActorDetails> searchByName(@RequestParam String name) {
        return mapStructMapper.actorsToActorsDetails(actorRepository.searchByName(name));
    }
}
